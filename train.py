...
def basic_model(arch):
    ...
    if arch == "vgg16":
        ...
        print('vgg16 or densenet only, defaulting now to vgg16')
        load_model = models.vgg16(pretrained=True)
...
def set_classifier(load_model, hidden_units):
    if hidden_units == None:
        hidden_units = 512
    input_num = load_model.classifier[0].in_features
    classifier = nn.Sequential(OrderedDict([('fc1', nn.Linear(input_num, hidden_units, bias=True)),
                                                  ('relu1', nn.ReLU()),
